jQuery(document).ready(function ($) {
    ...
    function initTimeInputs(scope = document) {
        ...
        const placeholder = wbbm_bus_edit.time_placeholder || '';
            ...
            success: function (response) {
                ...
                if (response.success) {
                    ...
                    if (response.data.status_label) {
                        ...
                        $('.bus-publish-warning').toggleClass('is-hidden', response.data.current_status === 'publish');
    ...
    $(document).on('click', '.add-inline-pickup-item', function () {
        ...
        const firstItem = list.find('.pickup-point-item').first();

        if (list.find('.pickup-point-item').length === 1) {
            const firstSelect = firstItem.find('select');
            const firstTime = firstItem.find('input[name^="wbbm_inline_pickpoint_time"]');

            if (firstSelect.length && !firstSelect.val() && firstTime.length && !firstTime.val()) {
        ...
        const html = `
            <div class="pickup-point-item">
                ...
                <div class="wbbm-time-field-wrap">
                    <input type="time" name="wbbm_inline_pickpoint_time[${stopIndex}][]" class="form-control sm wbbm-timepicker-field" value="" placeholder="${wbbm_bus_edit.time_placeholder || ''}">
                </div>
