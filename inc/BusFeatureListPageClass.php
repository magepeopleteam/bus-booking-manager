...
class BusFeatureListPageClass
{
    ...
    {
        add_action('admin_menu', array($this, 'register_bus_feature_list_page'), 15);
        add_action('admin_enqueue_scripts', array($this, 'enqueue_assets'), 20);
        add_action('admin_init', array($this, 'handle_bus_feature_actions'));
     ...
     */
    public function register_bus_feature_list_page()
    {
        add_submenu_page(
            'edit.php?post_type=wbbm_bus',
            __('Bus Feature', 'bus-booking-manager'),
            __('Bus Feature', 'bus-booking-manager'),
            'manage_options',
            'wbbm-bus-feature-list',
            array($this, 'render_bus_feature_list_page')
        );

        // Remove default taxonomy submenu
        remove_submenu_page('edit.php?post_type=wbbm_bus', 'edit-tags.php?taxonomy=wbbm_bus_feature&amp;post_type=wbbm_bus');
        remove_submenu_page('edit.php?post_type=wbbm_bus', 'edit-tags.php?taxonomy=wbbm_bus_feature&post_type=wbbm_bus');
    }
    ...
    public function reorder_bus_submenu()
    {
        ...
        $feature_page_slug = 'wbbm-bus-feature-list';

        $feature_item = false;
        foreach ($bus_menu as $key => $item) {
            if (isset($item[2]) && $item[2] === $feature_page_slug) {
                $feature_item = $item;
                unset($bus_menu[$key]);
                break;
            }
        ...
        if ($feature_item) {
            ...
            array_splice($bus_menu, 3, 0, array($feature_item));
    ...
    {
        global $pagenow;
        if ($pagenow === 'edit-tags.php' && isset($_GET['taxonomy']) && $_GET['taxonomy'] === 'wbbm_bus_feature' && !isset($_GET['action'])) {
            wp_safe_redirect(admin_url('edit.php?post_type=wbbm_bus&page=wbbm-bus-feature-list'));
            exit;
        }
    }
    ...
    {
        if (strpos($hook, 'wbbm-bus-feature-list') === false && (!isset($_GET['page']) || $_GET['page'] !== 'wbbm-bus-feature-list')) {
            return;
        }
     ...
     */
    public function handle_bus_feature_actions()
    {
        if (isset($_GET['page']) && $_GET['page'] === 'wbbm-bus-feature-list' && isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['term_id'])) {
            ...
            check_admin_referer('delete-bus-feature_' . $term_id);

            wp_delete_term($term_id, 'wbbm_bus_feature');

            wp_safe_redirect(admin_url('edit.php?post_type=wbbm_bus&page=wbbm-bus-feature-list&deleted=1'));
            exit;
        }
    }
     ...
     */
    public function render_bus_feature_list_page()
    {
        ...
        $total_terms = wp_count_terms(array('taxonomy' => 'wbbm_bus_feature', 'hide_empty' => false, 'search' => $s));
            ...
            <div class="wbbm-list-container-fullwidth">
                ...
                <div class="wbbm-list-header">
                    <div class="header-left">
                        ...
                        <div class="header-title-area">
                            <h2><?php _e('Bus Features', 'bus-booking-manager'); ?></h2>
                        </div>
                    </div>
                    <div class="header-right">
                        <a href="<?php echo admin_url('admin.php?post_type=wbbm_bus&page=wbbm-bus-feature-edit'); ?>" class="btn btn-primary">
                            <span class="dashicons dashicons-plus"></span> <?php _e('Add New Feature', 'bus-booking-manager'); ?>
                        </a>
                    </div>
                </div>
                ...
                <div class="wbbm-list-filters-card">
                    <form method="get" action="<?php echo admin_url('admin.php'); ?>" id="wbbm-list-filter-form">
                        ...
                        <input type="hidden" name="page" value="wbbm-bus-feature-list">

                        <div class="filters-row">
                            <div class="filter-left" style="flex-grow:initial">
                                <div class="filter-group search-group">
                                    <span class="dashicons dashicons-search"></span>
                                    <input type="text" name="s" value="<?php echo esc_attr($s); ?>" placeholder="<?php _e('Search features...', 'bus-booking-manager'); ?>" class="form-control">
                                </div>
                                <button type="submit" class="btn btn-primary btn-sm">
                                    <?php _e('Search', 'bus-booking-manager'); ?>
                                </button>
                                <?php if ($s) : ?>
                                    <a href="<?php echo admin_url('admin.php?post_type=wbbm_bus&page=wbbm-bus-feature-list'); ?>" class="btn btn-outline btn-sm" style="text-decoration:none;">
                    ...
                    <table class="wbbm-list-modern-table">
                        <thead>
                            <tr>
                                ...
                                <th class="col-bus" style="width: 300px;"><?php _e('Feature', 'bus-booking-manager'); ?></th>
                        ...
                        <tbody>
                                ...
                                <?php foreach ($terms as $term) :
                                    ...
                                    <tr>
                                        ...
                                        <td class="wbbm-list-info-cell">
                                            <div class="wbbm-list-icon" style="background: #f0f4f8; border-radius: 8px; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; margin-right: 15px;">
                                                <?php if ($icon) : ?>
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                ...
                                                <a href="<?php echo esc_url($delete_url); ?>" class="action-btn delete-btn wbbm-delete-bus-feature" title="Delete" onclick="return confirm('<?php esc_attr_e('Are you sure you want to delete this feature?', 'bus-booking-manager'); ?>');"><span class="dashicons dashicons-trash"></span></a>
                                            </div>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else : ?>
                                <tr>
                                    <td colspan="6" class="no-results"><?php _e('No features found.', 'bus-booking-manager'); ?></td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
