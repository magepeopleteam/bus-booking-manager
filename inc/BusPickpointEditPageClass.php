...
class BusPickpointEditPageClass
{
    ...
    {
        add_action('admin_menu', array($this, 'register_bus_pickpoint_edit_page'));
        add_action('admin_enqueue_scripts', array($this, 'enqueue_assets'), 20);
        add_action('admin_init', array($this, 'handle_bus_pickpoint_save'));
     ...
     */
    public function register_bus_pickpoint_edit_page()
    {
        add_submenu_page(
            'admin.php?post_type=wbbm_bus',
            __('Edit Pickup Point', 'bus-booking-manager'),
            __('Edit Pickup Point', 'bus-booking-manager'),
            'manage_options',
            'wbbm-bus-pickpoint-edit',
            array($this, 'render_bus_pickpoint_edit_page')
        );
    }
    ...
    {
        global $pagenow;
        if ($pagenow === 'term.php' && isset($_GET['taxonomy']) && $_GET['taxonomy'] === 'wbbm_bus_pickpoint') {
            $tag_id = isset($_GET['tag_ID']) ? intval($_GET['tag_ID']) : 0;
            wp_safe_redirect(admin_url('edit.php?post_type=wbbm_bus&page=wbbm-bus-pickpoint-edit&term_id=' . $tag_id));
            exit;
        }
    }
    ...
    {
        if (isset($_GET['page']) && $_GET['page'] === 'wbbm-bus-pickpoint-edit') {
    ...
    {
        if (isset($_GET['page']) && $_GET['page'] === 'wbbm-bus-pickpoint-edit') {
            $submenu_file = 'wbbm-bus-pickpoint-list';
        }
        return $submenu_file;
    }
    ...
    {
        if (strpos($hook, 'wbbm-bus-pickpoint-edit') === false && (!isset($_GET['page']) || $_GET['page'] !== 'wbbm-bus-pickpoint-edit')) {
            return;
        }
     ...
     */
    public function handle_bus_pickpoint_save()
    {
        if (isset($_POST['wbbm_bus_pickpoint_nonce']) && wp_verify_nonce($_POST['wbbm_bus_pickpoint_nonce'], 'wbbm_bus_pickpoint_save')) {
            ...
            if ($term_id) {
                wp_update_term($term_id, 'wbbm_bus_pickpoint', $args);
                $message = 'updated';
            } else {
                $term = wp_insert_term($name, 'wbbm_bus_pickpoint', $args);
                if (!is_wp_error($term)) {
     ...
     */
    public function render_bus_pickpoint_edit_page()
    {
        $term_id = isset($_GET['term_id']) ? intval($_GET['term_id']) : 0;
        $term = $term_id ? get_term($term_id, 'wbbm_bus_pickpoint') : null;
        ...
        <div class="wrap bus-edit-wrap">
            <div class="bus-edit-header">
                <div style="display: flex; align-items: center; gap: 15px;">
                    <a href="<?php echo admin_url('edit.php?post_type=wbbm_bus&page=wbbm-bus-pickpoint-list'); ?>" class="back-btn">
                        <span class="dashicons dashicons-arrow-left-alt"></span>
                        <?php _e('Back to List', 'bus-booking-manager'); ?>
                    </a>
                    <h2><?php echo $term_id ? __('Edit Pickup Point', 'bus-booking-manager') . ': ' . esc_html($name) : __('Add New Pickup Point', 'bus-booking-manager'); ?></h2>
                </div>
                <?php if ($term_id) : ?>
                    <div class="header-actions">
                        <a href="<?php echo admin_url('edit.php?post_type=wbbm_bus&page=wbbm-bus-pickpoint-edit'); ?>" class="btn btn-primary">
                            <span class="dashicons dashicons-plus-alt2"></span>
                            <?php _e('Add New Pickup Point', 'bus-booking-manager'); ?>
                        </a>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="bus-container">
                <form id="bus-pickpoint-edit-form" method="post" action="">
                    <?php wp_nonce_field('wbbm_bus_pickpoint_save', 'wbbm_bus_pickpoint_nonce'); ?>
                        ...
                        <div class="bus-edit-left">
                            <div class="bus-card">
                                <div class="form-group">
                                    ...
                                    <p class="description"><?php _e('The name of the pickup point.', 'bus-booking-manager'); ?></p>
                                </div>
                        ...
                        <div class="bus-edit-right">
                            <div class="bus-card">
                                ...
                                <div class="form-group" style="margin-top: 20px;">
                                    <button type="submit" id="bus-pickpoint-submit" class="btn btn-primary btn-block" style="width: 100%; justify-content: center;">
                                        <?php echo $term_id ? __('Update Pickup Point', 'bus-booking-manager') : __('Add New Pickup Point', 'bus-booking-manager'); ?>
                                    </button>
                                </div>
                            </div>
                        </div>
