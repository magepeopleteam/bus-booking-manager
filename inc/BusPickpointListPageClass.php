...
/**
 * Bus Pickpoint List Page Class
 * 
 * Handles the modern custom list page for bus pickup points (taxonomy wbbm_bus_pickpoint).
 */
class BusPickpointListPageClass
{
    public function __construct()
    {
        add_action('admin_menu', array($this, 'register_bus_pickpoint_list_page'), 22);
        add_action('admin_enqueue_scripts', array($this, 'enqueue_assets'), 20);
        add_action('admin_init', array($this, 'handle_bus_pickpoint_actions'));
        add_action('admin_init', array($this, 'handle_redirects'));
        add_action('admin_menu', array($this, 'reorder_bus_submenu'), 1002);
    }

    /**
     * Register the custom list page and replace default taxonomy entry
     */
    public function register_bus_pickpoint_list_page()
    {
        add_submenu_page(
            'edit.php?post_type=wbbm_bus',
            __('Pickup Points', 'bus-booking-manager'),
            __('Pickup Points', 'bus-booking-manager'),
            'manage_options',
            'wbbm-bus-pickpoint-list',
            array($this, 'render_bus_pickpoint_list_page')
        );

        // Remove default taxonomy submenu
        remove_submenu_page('edit.php?post_type=wbbm_bus', 'edit-tags.php?taxonomy=wbbm_bus_pickpoint&amp;post_type=wbbm_bus');
        remove_submenu_page('edit.php?post_type=wbbm_bus', 'edit-tags.php?taxonomy=wbbm_bus_pickpoint&post_type=wbbm_bus');
    }
    ...
    public function reorder_bus_submenu()
    {
        ...
        $pickpoint_page_slug = 'wbbm-bus-pickpoint-list';

        $pickpoint_item = false;
        foreach ($bus_menu as $key => $item) {
            if (isset($item[2]) && $item[2] === $pickpoint_page_slug) {
                $pickpoint_item = $item;
                unset($bus_menu[$key]);
                break;
            }
        ...
        if ($pickpoint_item) {
            ...
            // Insert after Bus Stops (which should be at index 2 or 3)
            // Let's find Bus Stops and insert after it, or just place it at index 3
            array_splice($bus_menu, 3, 0, array($pickpoint_item));
    ...
    {
        global $pagenow;
        if ($pagenow === 'edit-tags.php' && isset($_GET['taxonomy']) && $_GET['taxonomy'] === 'wbbm_bus_pickpoint' && !isset($_GET['action'])) {
            wp_safe_redirect(admin_url('edit.php?post_type=wbbm_bus&page=wbbm-bus-pickpoint-list'));
            exit;
        }
    }
    ...
    {
        if (strpos($hook, 'wbbm-bus-pickpoint-list') === false && (!isset($_GET['page']) || $_GET['page'] !== 'wbbm-bus-pickpoint-list')) {
            return;
        }
     ...
     */
    public function handle_bus_pickpoint_actions()
    {
        if (isset($_GET['page']) && $_GET['page'] === 'wbbm-bus-pickpoint-list' && isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['term_id'])) {
            ...
            check_admin_referer('delete-bus-pickpoint_' . $term_id);

            wp_delete_term($term_id, 'wbbm_bus_pickpoint');

            wp_safe_redirect(admin_url('edit.php?post_type=wbbm_bus&page=wbbm-bus-pickpoint-list&deleted=1'));
            exit;
        }
    }
     ...
     */
    public function render_bus_pickpoint_list_page()
    {
        ...
        $total_terms = wp_count_terms(array('taxonomy' => 'wbbm_bus_pickpoint', 'hide_empty' => false, 'search' => $s));
            ...
            <div class="shuttle-list-container-fullwidth">
                ...
                <div class="shuttle-list-header">
                    <div class="header-left">
                        ...
                        <div class="header-title-area">
                            <h2><?php _e('Pickup Points', 'bus-booking-manager'); ?></h2>
                        </div>
                    </div>
                    <div class="header-right">
                        <a href="<?php echo admin_url('admin.php?post_type=wbbm_bus&page=wbbm-bus-pickpoint-edit'); ?>" class="btn btn-primary">
                            <span class="dashicons dashicons-plus"></span> <?php _e('Add New Pickup Point', 'bus-booking-manager'); ?>
                        </a>
                    </div>
                </div>
                ...
                <div class="shuttle-filters-card">
                    <form method="get" action="<?php echo admin_url('admin.php'); ?>" id="shuttle-list-filter-form">
                        ...
                        <input type="hidden" name="page" value="wbbm-bus-pickpoint-list">

                        <div class="filters-row">
                            <div class="filter-left" style="flex-grow:initial">
                                <div class="filter-group search-group">
                                    <span class="dashicons dashicons-search"></span>
                                    <input type="text" name="s" value="<?php echo esc_attr($s); ?>" placeholder="<?php _e('Search pickup points...', 'bus-booking-manager'); ?>" class="form-control">
                                </div>
                                <button type="submit" class="btn btn-primary btn-sm">
                                    <?php _e('Search', 'bus-booking-manager'); ?>
                                </button>
                                <?php if ($s) : ?>
                                    <a href="<?php echo admin_url('admin.php?post_type=wbbm_bus&page=wbbm-bus-pickpoint-list'); ?>" class="btn btn-outline btn-sm" style="text-decoration:none;">
                        ...
                        <tbody>
                            <?php if (!empty($terms) && !is_wp_error($terms)) : ?>
                                <?php foreach ($terms as $term) :
                                    $edit_url = admin_url('admin.php?post_type=wbbm_bus&page=wbbm-bus-pickpoint-edit&term_id=' . $term->term_id);
                                    $delete_url = wp_nonce_url(add_query_arg(array('action' => 'delete', 'term_id' => $term->term_id)), 'delete-bus-pickpoint_' . $term->term_id);
                                ?>
                                    <tr>
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                ...
                                                <a href="<?php echo esc_url($delete_url); ?>" class="action-btn delete-btn wbbm-delete-bus-pickpoint" title="Delete" onclick="return confirm('<?php esc_attr_e('Are you sure you want to delete this pickup point?', 'bus-booking-manager'); ?>');"><span class="dashicons dashicons-trash"></span></a>
                                            </div>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else : ?>
                                <tr>
                                    <td colspan="6" class="no-results"><?php _e('No pickup points found.', 'bus-booking-manager'); ?></td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
