...
/**
 * Shuttle Category Edit Page Class
 * 
 * Handles the custom modern edit/add page for shuttle categories (taxonomy wbbm_shuttle_cat).
 */
class ShuttleCatEditPageClass
{
    public function __construct()
    {
        add_action('admin_menu', array($this, 'register_shuttle_cat_edit_page'));
        add_action('admin_enqueue_scripts', array($this, 'enqueue_assets'), 20);
        add_action('admin_init', array($this, 'handle_shuttle_cat_save'));
        ...
        add_action('admin_menu', function () {
            remove_submenu_page(
                'admin.php?post_type=wbbm_shuttle',
                'wbbm-shuttle-cat-edit'
            );
        }, 999);
    }
     ...
     */
    public function register_shuttle_cat_edit_page()
    {
        add_submenu_page(
            'admin.php?post_type=wbbm_shuttle',
            __('Edit Category', 'bus-booking-manager'),
            __('Edit Category', 'bus-booking-manager'),
            'manage_options',
            'wbbm-shuttle-cat-edit',
            array($this, 'render_shuttle_cat_edit_page')
        );
    }
    ...
    {
        global $pagenow;
        if ($pagenow === 'term.php' && isset($_GET['taxonomy']) && $_GET['taxonomy'] === 'wbbm_shuttle_cat') {
            $tag_id = isset($_GET['tag_ID']) ? intval($_GET['tag_ID']) : 0;
            wp_safe_redirect(admin_url('edit.php?post_type=wbbm_shuttle&page=wbbm-shuttle-cat-edit&term_id=' . $tag_id));
            exit;
        }
    }
    ...
    {
        if (isset($_GET['page']) && $_GET['page'] === 'wbbm-shuttle-cat-edit') {
    ...
    {
        if (isset($_GET['page']) && $_GET['page'] === 'wbbm-shuttle-cat-edit') {
            $submenu_file = 'wbbm-shuttle-cat-list';
        }
        return $submenu_file;
    }
    ...
    {
        if (strpos($hook, 'wbbm-shuttle-cat-edit') === false && (!isset($_GET['page']) || $_GET['page'] !== 'wbbm-shuttle-cat-edit')) {
            return;
        }
     ...
     */
    public function handle_shuttle_cat_save()
    {
        if (isset($_POST['wbbm_shuttle_cat_nonce']) && wp_verify_nonce($_POST['wbbm_shuttle_cat_nonce'], 'wbbm_shuttle_cat_save')) {
            ...
            if ($term_id) {
                wp_update_term($term_id, 'wbbm_shuttle_cat', $args);
                $message = 'updated';
            } else {
                $term = wp_insert_term($name, 'wbbm_shuttle_cat', $args);
                if (!is_wp_error($term)) {
     ...
     */
    public function render_shuttle_cat_edit_page()
    {
        $term_id = isset($_GET['term_id']) ? intval($_GET['term_id']) : 0;
        $term = $term_id ? get_term($term_id, 'wbbm_shuttle_cat') : null;
        ...
        <div class="wrap shuttle-edit-wrap">
            <div class="shuttle-edit-header">
                <div style="display: flex; align-items: center; gap: 15px;">
                    <a href="<?php echo admin_url('edit.php?post_type=wbbm_shuttle&page=wbbm-shuttle-cat-list'); ?>" class="back-btn">
                        <span class="dashicons dashicons-arrow-left-alt"></span>
                        <?php _e('Back to List', 'bus-booking-manager'); ?>
                    </a>
                    <h2><?php echo $term_id ? __('Edit Category', 'bus-booking-manager') . ': ' . esc_html($name) : __('Add New Category', 'bus-booking-manager'); ?></h2>
                </div>
                <?php if ($term_id) : ?>
                    <div class="header-actions">
                        <a href="<?php echo admin_url('edit.php?post_type=wbbm_shuttle&page=wbbm-shuttle-cat-edit'); ?>" class="btn btn-primary">
                            <span class="dashicons dashicons-plus-alt2"></span>
                            <?php _e('Add New Category', 'bus-booking-manager'); ?>
                        </a>
                    </div>
                <?php endif; ?>
            </div>

            <div class="shuttle-container" style="padding:0">
                <form id="shuttle-cat-edit-form" method="post" action="">
                    <?php wp_nonce_field('wbbm_shuttle_cat_save', 'wbbm_shuttle_cat_nonce'); ?>
                        ...
                        <div class="shuttle-edit-right">
                            <div class="shuttle-card">
                                ...
                                <div class="form-group" style="margin-top: 20px;">
                                    <button type="submit" id="shuttle-cat-submit" class="btn btn-primary btn-block" style="width: 100%; justify-content: center;">
                                        <?php echo $term_id ? __('Update Category', 'bus-booking-manager') : __('Add New Category', 'bus-booking-manager'); ?>
                                    </button>
                                </div>
                            </div>
                        </div>
