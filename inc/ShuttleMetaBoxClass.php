    ...
    ?>
        <div id="wbbm_shuttle_basic" class="mp_tab_item tab-content active" data-tab-item="#wbbm_shuttle_basic">
            ...
            <div class="mp_settings_panel">
                <div class="mp_settings_panel_body w-100">

                    <div class="d-grid col-3 gap-2">
                        <!-- Shuttle Type -->
                        <div class="mpStyle border-box mb-0">
            ...
            <div class="wbbm_route_body">
                <div class="wbbm_route_fields_grid">
                    ...
                    <div class="wbbm_shuttle_field">
                        ...
                        <p class="description" style="margin-top:5px;"><?php echo esc_html(__('Select "Round Trip" if this route allows return booking. You must define a separate route for the return journey.', 'bus-booking-manager')); ?></p>
                    </div>
                </div>
...
<?php
    ...
    public function wbbm_shuttle_meta_save($post_id, $post)
    {
        // Security checks
        if (
            !isset($_POST['wbbm_shuttle_settings_nonce']) ||
        ...
        if (isset($_POST['wbbm_shuttle_schedule']) && is_array($_POST['wbbm_shuttle_schedule'])) {
            foreach ($_POST['wbbm_shuttle_schedule'] as $route_id => $route_data) {
                if (is_array($route_data)) {
                    // Check if structured (forward/return) or flat (legacy)
                    if (isset($route_data['forward']) || isset($route_data['return'])) {
                        foreach (['forward', 'return'] as $direction) {
                            if (isset($route_data[$direction]) && is_array($route_data[$direction])) {
                                foreach ($route_data[$direction] as $data) {
                                    if (!empty($data['time'])) {
                                        $schedule[sanitize_text_field($route_id)][$direction][] = array(
                        ...
                        // Flat handling (legacy/one-way) -> map to 'forward'
                        foreach ($route_data as $data) {
                            if (!empty($data['time'])) {
                                $schedule[sanitize_text_field($route_id)]['forward'][] = array(
                                    'time' => sanitize_text_field($data['time']),
                                    'days' => isset($data['days']) ? array_map('sanitize_text_field', $data['days']) : array()
                                );
                            }
