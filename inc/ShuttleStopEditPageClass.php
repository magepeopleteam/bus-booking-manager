...
/**
 * Shuttle Stop Edit Page Class
 * 
 * Handles the custom modern edit/add page for shuttle stops (taxonomy wbbm_shuttle_stops).
 */
class ShuttleStopEditPageClass
{
    public function __construct()
    {
        add_action('admin_menu', array($this, 'register_shuttle_stop_edit_page'));
        add_action('admin_enqueue_scripts', array($this, 'enqueue_assets'), 20);
        add_action('admin_init', array($this, 'handle_shuttle_stop_save'));
     ...
     */
    public function register_shuttle_stop_edit_page()
    {
        add_submenu_page(
            'admin.php?post_type=wbbm_shuttle',
            __('Edit Stop', 'bus-booking-manager'),
            __('Edit Stop', 'bus-booking-manager'),
            'manage_options',
            'wbbm-shuttle-stop-edit',
            array($this, 'render_shuttle_stop_edit_page')
        );
    }
    ...
    {
        global $pagenow;
        if ($pagenow === 'term.php' && isset($_GET['taxonomy']) && $_GET['taxonomy'] === 'wbbm_shuttle_stops') {
            $tag_id = isset($_GET['tag_ID']) ? intval($_GET['tag_ID']) : 0;
            wp_safe_redirect(admin_url('edit.php?post_type=wbbm_shuttle&page=wbbm-shuttle-stop-edit&term_id=' . $tag_id));
            exit;
        }
    }
    ...
    {
        if (isset($_GET['page']) && $_GET['page'] === 'wbbm-shuttle-stop-edit') {
    ...
    {
        if (isset($_GET['page']) && $_GET['page'] === 'wbbm-shuttle-stop-edit') {
            $submenu_file = 'wbbm-shuttle-stop-list';
        }
        return $submenu_file;
    }
    ...
    {
        if (strpos($hook, 'wbbm-shuttle-stop-edit') === false && (!isset($_GET['page']) || $_GET['page'] !== 'wbbm-shuttle-stop-edit')) {
            return;
        }
     ...
     */
    public function handle_shuttle_stop_save()
    {
        if (isset($_POST['wbbm_shuttle_stop_nonce']) && wp_verify_nonce($_POST['wbbm_shuttle_stop_nonce'], 'wbbm_shuttle_stop_save')) {
            ...
            if ($term_id) {
                wp_update_term($term_id, 'wbbm_shuttle_stops', $args);
                $message = 'updated';
            } else {
                $term = wp_insert_term($name, 'wbbm_shuttle_stops', $args);
                if (!is_wp_error($term)) {
     ...
     */
    public function render_shuttle_stop_edit_page()
    {
        $term_id = isset($_GET['term_id']) ? intval($_GET['term_id']) : 0;
        $term = $term_id ? get_term($term_id, 'wbbm_shuttle_stops') : null;
        ...
        <div class="wrap shuttle-edit-wrap">
            <div class="shuttle-edit-header">
                <div style="display: flex; align-items: center; gap: 15px;">
                    <a href="<?php echo admin_url('edit.php?post_type=wbbm_shuttle&page=wbbm-shuttle-stop-list'); ?>" class="back-btn">
                        <span class="dashicons dashicons-arrow-left-alt"></span>
                        <?php _e('Back to List', 'bus-booking-manager'); ?>
                    </a>
                    <h2><?php echo $term_id ? __('Edit Shuttle Stop', 'bus-booking-manager') . ': ' . esc_html($name) : __('Add New Shuttle Stop', 'bus-booking-manager'); ?></h2>
                </div>
                <?php if ($term_id) : ?>
                    <div class="header-actions">
                        <a href="<?php echo admin_url('edit.php?post_type=wbbm_shuttle&page=wbbm-shuttle-stop-edit'); ?>" class="btn btn-primary">
            ...
            <div class="shuttle-container" style="padding:0">
                <form id="shuttle-stop-edit-form" method="post" action="">
                    <?php wp_nonce_field('wbbm_shuttle_stop_save', 'wbbm_shuttle_stop_nonce'); ?>
                        ...
                        <div class="shuttle-edit-right">
                            <div class="shuttle-card">
                                ...
                                <div class="form-group" style="margin-top: 20px;">
                                    <button type="submit" id="shuttle-stop-submit" class="btn btn-primary btn-block" style="width: 100%; justify-content: center;">
                                        <?php echo $term_id ? __('Update Shuttle Stop', 'bus-booking-manager') : __('Add New Shuttle Stop', 'bus-booking-manager'); ?>
                                    </button>
                                </div>
                            </div>
                        </div>
