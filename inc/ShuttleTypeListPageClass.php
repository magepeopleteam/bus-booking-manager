...
/**
 * Shuttle Type List Page Class
 *
 * Handles the modern custom list page for shuttle types (taxonomy wbbm_shuttle_type).
 */
class ShuttleTypeListPageClass
{
    public function __construct()
    {
        add_action('admin_menu', array($this, 'register_shuttle_type_list_page'), 20);
        add_action('admin_enqueue_scripts', array($this, 'enqueue_assets'), 20);
        add_action('admin_init', array($this, 'handle_shuttle_type_actions'));
        add_action('admin_init', array($this, 'handle_redirects'));
        add_action('admin_menu', array($this, 'reorder_shuttle_submenu'), 1000);
    }

    /**
     * Reorder shuttle submenu to keep "Shuttle Types" as 2nd item (index 2)
     */
    public function reorder_shuttle_submenu()
    {
        ...
        $type_page_slug = 'wbbm-shuttle-type-list';
        ...
        if ($type_item) {
            ...
            array_splice($shuttle_menu, 1, 0, array($type_item));
     ...
     */
    public function register_shuttle_type_list_page()
    {
        add_submenu_page(
            'edit.php?post_type=wbbm_shuttle',
            __('Shuttle Types', 'bus-booking-manager'),
            __('Shuttle Types', 'bus-booking-manager'),
            'manage_options',
            'wbbm-shuttle-type-list',
            array($this, 'render_shuttle_type_list_page')
        );

        // Remove default taxonomy submenu
        remove_submenu_page('edit.php?post_type=wbbm_shuttle', 'edit-tags.php?taxonomy=wbbm_shuttle_type&amp;post_type=wbbm_shuttle');
        remove_submenu_page('edit.php?post_type=wbbm_shuttle', 'edit-tags.php?taxonomy=wbbm_shuttle_type&post_type=wbbm_shuttle');
    }
    ...
    {
        global $pagenow;
        if ($pagenow === 'edit-tags.php' && isset($_GET['taxonomy']) && $_GET['taxonomy'] === 'wbbm_shuttle_type' && !isset($_GET['action'])) {
            wp_safe_redirect(admin_url('edit.php?post_type=wbbm_shuttle&page=wbbm-shuttle-type-list'));
            exit;
        }
    }
    ...
    {
        if (strpos($hook, 'wbbm-shuttle-type-list') === false && (!isset($_GET['page']) || $_GET['page'] !== 'wbbm-shuttle-type-list')) {
            return;
        }
     ...
     */
    public function handle_shuttle_type_actions()
    {
        if (isset($_GET['page']) && $_GET['page'] === 'wbbm-shuttle-type-list' && isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['term_id'])) {
            ...
            check_admin_referer('delete-shuttle-type_' . $term_id);

            wp_delete_term($term_id, 'wbbm_shuttle_type');

            wp_safe_redirect(admin_url('edit.php?post_type=wbbm_shuttle&page=wbbm-shuttle-type-list&deleted=1'));
            exit;
        }
    }
     ...
     */
    public function render_shuttle_type_list_page()
    {
        ...
        $total_terms = wp_count_terms(array('taxonomy' => 'wbbm_shuttle_type', 'hide_empty' => false, 'search' => $s));
            ...
            <div class="shuttle-list-container">
                ...
                <div class="shuttle-list-header">
                    <div class="header-left">
                        ...
                        <div class="header-title-area">
                            <h2><?php _e('Shuttle Types', 'bus-booking-manager'); ?></h2>
                        </div>
                    </div>
                    <div class="header-right">
                        <a href="<?php echo admin_url('admin.php?post_type=wbbm_shuttle&page=wbbm-shuttle-type-edit'); ?>" class="btn btn-primary">
                            <span class="dashicons dashicons-plus"></span> <?php _e('Add New Shuttle Type', 'bus-booking-manager'); ?>
                        </a>
                    </div>
                </div>
                ...
                <div class="shuttle-filters-card">
                    <form method="get" action="<?php echo admin_url('admin.php'); ?>" id="shuttle-list-filter-form">
                        ...
                        <input type="hidden" name="page" value="wbbm-shuttle-type-list">

                        <div class="filters-row">
                            <div class="filter-left" style="flex-grow:initial">
                                <div class="filter-group search-group">
                                    <span class="dashicons dashicons-search"></span>
                                    <input type="text" name="s" value="<?php echo esc_attr($s); ?>" placeholder="<?php _e('Search shuttle types...', 'bus-booking-manager'); ?>" class="form-control">
                                </div>
                                <button type="submit" class="btn btn-primary btn-sm">
                                    <?php _e('Search', 'bus-booking-manager'); ?>
                                </button>
                                <?php if ($s) : ?>
                                    <a href="<?php echo admin_url('admin.php?post_type=wbbm_shuttle&page=wbbm-shuttle-type-list'); ?>" class="btn btn-outline btn-sm" style="text-decoration:none;">
                        ...
                        <tbody>
                            <?php if (!empty($terms) && !is_wp_error($terms)) : ?>
                                <?php foreach ($terms as $term) :
                                    $edit_url = admin_url('admin.php?post_type=wbbm_shuttle&page=wbbm-shuttle-type-edit&term_id=' . $term->term_id);
                                    $delete_url = wp_nonce_url(add_query_arg(array('action' => 'delete', 'term_id' => $term->term_id)), 'delete-shuttle-type_' . $term->term_id);
                                    $filter_url = admin_url('edit.php?post_type=wbbm_shuttle&page=wbbm-shuttle-list&wbbm_shuttle_type=' . $term->slug);
                                    ?>
                                    <tr>
                                        ...
                                        <td>
                                            <div class="capacity-info">
                                                <a href="<?php echo esc_url($filter_url); ?>" class="count" title="<?php esc_attr_e('View shuttles with this type', 'bus-booking-manager'); ?>"><?php echo esc_html($term->count); ?></a>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="action-buttons">
                                                <a href="<?php echo esc_url($edit_url); ?>" class="action-btn" title="Edit"><span class="dashicons dashicons-edit"></span></a>
                                                <a href="<?php echo esc_url($delete_url); ?>" class="action-btn delete-btn wbbm-delete-shuttle-type" title="Delete" onclick="return confirm('<?php esc_attr_e('Are you sure you want to delete this shuttle type?', 'bus-booking-manager'); ?>');"><span class="dashicons dashicons-trash"></span></a>
                                            </div>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else : ?>
                                <tr>
                                    <td colspan="6" class="no-results"><?php _e('No shuttle types found.', 'bus-booking-manager'); ?></td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
