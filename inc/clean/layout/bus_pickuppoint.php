<div class="mp_tab_item" data-tab-item="#wbtm_pickuppoint">
    <h3 class="wbbm_mp_tab_item_heading"><?php echo esc_html__('Pickup Point Settings', 'bus-booking-manager'); ?></h3>
    <p><?php echo esc_html__('Here you can set pickup point', 'bus-booking-manager'); ?></p>
            ...
            <div class="wbbm_bus_pickpint_wrapper">
                <section>
                    ...
                    <div class="wbbm_field_group <?php echo esc_attr($boarding_points_class); ?>">
                        ...
                        <select name="wbbm_pick_boarding" id="wbbm_pick_boarding" class="ra_pick_boarding"></select>
                ...
                <section class="wbbm_right_col <?php echo ($selected_city_pickpoints == '' ? 'all-center' : ''); ?>">
                    <div id="wbbm_pickpoint_selected_city" style="margin:auto;">
                        <?php if (!empty($selected_city_pickpoints)) {
                            $selected_city_pickpoints = explode(',', $selected_city_pickpoints);
                            foreach ($selected_city_pickpoints as $single) {
                                $city_name = $single ? ucfirst(str_replace('_', ' ', sanitize_text_field($single))) : '';
                                $get_pickpoints_data = get_post_meta($post->ID, 'wbbm_selected_pickpoint_name_' . strtolower(sanitize_text_field($single)), true);
                                ?>
                                            ...
                                            <?php if ($get_pickpoints_data) {
                                                foreach ($get_pickpoints_data as $pickpoint) { ?>
                                                    <div class="pickpoint-adding">
                                                        ...
                                                        <button class="wbbm_remove_pickpoint"><i class="fas fa-minus-circle"></i> <?php esc_html_e('Remove', 'bus-booking-manager'); ?></button>
                                                    </div>
                                                <?php }
                                            } ?>
                                        </div>
                            ...
                            echo '<p class="blank-pickpoint" style="color: #FF9800;font-weight: 700;text-align:left">' . esc_html__('No pickup point added yet!', 'bus-booking-manager') . '</p>';
                        }
                        ?>
                    </div>
                </section>
                ...
                jQuery(document).ready(function($) {
                    $(document).on('click', '.wbbm_add_pickpoint_this_city', function(e) {
                        ...
                        var html =
                            ...
                            '<div class="pickpoint-adding-col">' +
                            '<span class="wbbm_bus_route_icon wbbm_bus_route_icon1"><img src="<?php echo esc_url(WBTM_PLUGIN_URL .'images/bus_route_map.png');?>"/></span>' +
                            '<select class="pickup_add_option" name="wbbm_selected_pickpoint_name_' + get_boarding_point + '[]">' + pickuppoints_first + '</select>' +
                            '</div>' +
                            '<div class="pickpoint-adding-col">' +
                            '<span class="wbbm_bus_route_icon wbbm_bus_route_icon2"><img src="<?php echo esc_url(WBTM_PLUGIN_URL .'images/bus_route_clock.png');?>"/></span>' +
                            '<input type="text" data-clocklet name="wbbm_selected_pickpoint_time_' + get_boarding_point + '[]" placeholder="15:00">' +
                            '</div>' +
                            '<button class="wbbm_remove_pickpoint"><i class="fas fa-minus-circle"></i> <?php esc_html_e("Remove", "bus-booking-manager"); ?></button>' +
                            '</div></div>' +
                            '<button class="wbbm_add_more_pickpoint"><i class="fas fa-plus"></i> <?php esc_html_e("Add more", "bus-booking-manager"); ?></button>' +
                        ...
                        $('#wbbm_pick_boarding option:first').prop('selected', true);
                    });
                    ...
                    $(document).on('click', '.remove_city_for_pickpoint', function(e) {
                        ...
                        $('#wbbm_pick_boarding').append('<option value="' + city_name_val + '">' + city_name + '</option>');
                    ...
                    $(document).on('click', '.wbbm_add_more_pickpoint', function(e) {
                        ...
                        $adding_more = $(this).siblings('.pickpoint-adding-wrap').find('.pickpoint-adding:first').clone(true);
                    ...
                    $(document).on('click', '.wbbm_remove_pickpoint', function(e) {
                        ...
                        if ($(this).parents('.pickpoint-adding-wrap').children().length == 1) {
                            $(this).parents('.wbbm_selected_city_item').find('.remove_city_for_pickpoint').trigger('click');
                        }
