	...
	function wbbm_after_checkout_validation() {
		...
		if (sizeof($cart_items) > 0) {
			foreach ($cart_items as $cart_item) {
				$post_id = array_key_exists('wbbm_id', $cart_item) ? $cart_item['wbbm_id'] : 0;
				if (get_post_type($post_id) == 'wbbm_bus') {
					$start_route = array_key_exists('wbbm_start_stops', $cart_item) ? $cart_item['wbbm_start_stops'] : '';
					$end_route = array_key_exists('wbbm_end_stops', $cart_item) ? $cart_item['wbbm_end_stops'] : '';
