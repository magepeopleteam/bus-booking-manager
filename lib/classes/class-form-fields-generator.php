...
if (! class_exists('FormFieldsGenerator')) {
    ...
    class FormFieldsGenerator
    {
        ...
        public function field_post_objects($option)
        {
            ...
            ob_start();
            ?>
            <div <?php if (!empty($depends)) {
                ?> data-depends="[<?php echo esc_attr($depends); ?>]" <?php
                 } ?>
                    id="field-wrapper-<?php esc_attr($id); ?>" class="<?php if (!empty($depends)) {
    ...
    echo 'dependency-field';
                     } ?> field-wrapper field-margin-wrapper field-margin-wrapper-<?php esc_attr($id); ?>">
                    <label><input type="checkbox" class="change-together">Apply for all</label>
                    <div class="item-list">
                        <?php
                        foreach ($args as $index => $arg) :
...
// Verify the token on the server.
    ...
    echo 'dependency-field';
                 } ?> field-wrapper field-url-wrapper field-url-wrapper-<?php esc_attr($id); ?>">
...
<!-- <span index_id="--><?php //echo esc_attr($index); ?><!--" class="button clone"><i class="far fa-clone"></i></span>-->
        ...
        public function get_tax_data($args)
        {
            foreach ($this->get_rep_taxonomies_array($args) as $argIndex => $argName) :
                $selected = ($argIndex == $value) ? 'selected' : ''; ?><option <?php echo esc_attr($selected); ?>  value="<?php echo esc_attr($argIndex); ?>"><?php echo esc_attr($argName); ?></option> <?php
            endforeach;
        }
        ...
        public function get_rep_taxonomies_array($string)
        {
            ...
            if (! taxonomy_exists($taxonomy)) {
                throw new Pick_error(wp_kses_post("Taxonomy <strong>$taxonomy</strong> doesn't exists !"));
            }
