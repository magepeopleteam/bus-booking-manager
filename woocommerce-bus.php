...
?>
	...
	add_action('woocommerce_checkout_order_processed', 'wbbm_order_status_before_payment_without_gutenberg', 10, 3);
	add_action('woocommerce_store_api_checkout_order_processed', 'wbbm_order_status_before_payment', 10, 3);
	function wbbm_order_status_before_payment($order)
	{
		$order->update_status('processing');
	}
	function wbbm_order_status_before_payment_without_gutenberg($order_id, $posted_data, $order)
	{
		$order->update_status('processing');
	}
	...
	add_action('woocommerce_checkout_order_processed', 'wbbm_add_passenger_to_db_without_gutenberg', 10, 3);
	add_action('woocommerce_store_api_checkout_order_processed', 'wbbm_add_passenger_to_db', 1, 1);

	function wbbm_add_passenger_to_db($order)
	{
		...
		error_log('Billing Info: ' . print_r($billing_info, true));
		$user_name = $billing_info['first_name'] . ' ' . $billing_info['last_name'];
		$user_email = $billing_info['email'];
		$user_phone = $billing_info['phone'];
		$user_address = $billing_info['address_1'] . ', ' . $billing_info['city'] . ', ' . $billing_info['postcode'];
		$user_gender = ""; // Not in billing, can be skipped
		$user_dob = "";    // Not in billing, can be skipped
		$nationality = $billing_info['country']; // Using country as nationality
		$flight_arrival_no = ""; // No data in billing, skipped
		$flight_departure_no = ""; // No data in billing, skipped
		$extra_bag_quantity = ""; // No data in billing, skipped
		$user_type = "Adult"; // Defaulting to 'Adult'

		$order_meta = get_post_meta($order_id);

		if ($order) {
			$order_status = $order->get_status();
			$status = ($order_status == 'processing' || $order_status == 'completed') ? 1 : 0;
			...
			foreach ($order->get_items() as $item_id => $item_values) {
				...
				if (get_post_type($eid) == 'wbbm_bus') {
					...
					$item_quantity = $adult + ($child ?? 0) + ($infant ?? 0) + ($entire ?? 0);
	...
	function wbbm_add_passenger_to_db_without_gutenberg($order_id)
	{
		global $wpdb;
		...
		if ($order) {
			...
			foreach ($order->get_items() as $item_id => $item_values) {
				...
				if (get_post_type($eid) == 'wbbm_bus') {
					...
					$bus_id = wbbm_get_order_meta($item_id, '_bus_id');
					$b_time = wbbm_get_order_meta($item_id, '_btime');
					error_log('b time',$b_time);
					error_log('j time',$j_time);
					$adult = wbbm_get_order_meta($item_id, '_Adult');
					$child = wbbm_get_order_meta($item_id, '_Child');
					$infant = wbbm_get_order_meta($item_id, '_Infant');
					$entire = wbbm_get_order_meta($item_id, '_Entire');
					...
					for ($x = 1; $x <= $item_quantity; $x++) {
						...
						if (empty($user_name) || empty($user_email) || empty($user_phone)) {
							...
							$order = wc_get_order($order_id);
							if ($order) {
								// Populate user information from order billing data
								$first_name = $order->get_billing_first_name();
								$last_name = $order->get_billing_last_name();
								$user_name = empty($user_name) ? $first_name . ' ' . $last_name : $user_name;
								$user_email = empty($user_email) ? $order->get_billing_email() : $user_email;
								$user_phone = empty($user_phone) ? $order->get_billing_phone() : $user_phone;
								$user_address = empty($user_address) ? $order->get_billing_address_1() . ' ' . $order->get_billing_address_2() : $user_address;
							}
						}
						$check_before_add = wbbm_get_order_seat_check($bus_id, $order_id, $user_type, $b_time, $j_date);
						if ($check_before_add == 0) {
					...
					error_log('User Name: ' . print_r($user_name, true));
			...
			foreach ($order->get_items() as $item_id => $item_values) {
				...
				if (get_post_type($eid) == 'wbbm_bus') {
					...
					if ($order->has_status('cancelled')) {
						...
						$wpdb->query(
							$wpdb->prepare(
								"UPDATE $table_name SET status = %d, ticket_status = %s WHERE order_id = %d AND bus_id = %d",
								$status,
								$ticket_status,
								$order_id,
								$event_id
